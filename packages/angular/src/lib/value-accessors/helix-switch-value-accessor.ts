...
/**
 * `ControlValueAccessor` for the `<hx-switch>` web component.
 *
 * Bridges Angular reactive/template-driven forms with `hx-switch`.
 ...
 */
@Directive({
  selector: 'hx-switch[formControlName], hx-switch[formControl], hx-switch[ngModel]',
  standalone: true,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => HelixSwitchValueAccessor),
      multi: true,
    },
  ],
})
export class HelixSwitchValueAccessor implements ControlValueAccessor {
