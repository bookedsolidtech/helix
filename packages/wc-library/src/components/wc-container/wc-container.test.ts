...
import type { WcContainer } from './wc-container.js';
import './index.js';
...
describe('wc-container', () => {
  ...
  describe('Rendering', () => {
    it('renders with shadow DOM', async () => {
      const el = await fixture<WcContainer>('<wc-container>Content</wc-container>');
  ...
  describe('Property: padding', () => {
    ...
    it('padding="sm" reflected as attribute', async () => {
      const el = await fixture<WcContainer>('<wc-container padding="sm">Content</wc-container>');
      expect(el.getAttribute('padding')).toBe('sm');
    });

    it('padding="md" reflected as attribute', async () => {
      const el = await fixture<WcContainer>('<wc-container padding="md">Content</wc-container>');
      expect(el.getAttribute('padding')).toBe('md');
    });

    it('padding="lg" reflected as attribute', async () => {
      const el = await fixture<WcContainer>('<wc-container padding="lg">Content</wc-container>');
      expect(el.getAttribute('padding')).toBe('lg');
    });

    it('padding="xl" reflected as attribute', async () => {
      const el = await fixture<WcContainer>('<wc-container padding="xl">Content</wc-container>');
      expect(el.getAttribute('padding')).toBe('xl');
    });

    it('padding="2xl" reflected as attribute', async () => {
      const el = await fixture<WcContainer>('<wc-container padding="2xl">Content</wc-container>');
  ...
  describe('Slot: default', () => {
    it('slotted content renders', async () => {
      const el = await fixture<WcContainer>('<wc-container><p>Hello World</p></wc-container>');
    ...
    it('multiple children render', async () => {
      const el = await fixture<WcContainer>(
        '<wc-container><p>First</p><p>Second</p><p>Third</p></wc-container>'
      );
  ...
  describe('CSS Custom Properties', () => {
    it('--wc-container-bg applies background color to :host', async () => {
      const el = await fixture<WcContainer>(
        '<wc-container style="--wc-container-bg: rgb(255, 0, 0);">Content</wc-container>'
      );
      const styles = getComputedStyle(el);
      expect(styles.backgroundColor).toBe('rgb(255, 0, 0)');
    });

    it('--wc-container-gutter applies horizontal padding on inner', async () => {
      const el = await fixture<WcContainer>(
        '<wc-container style="--wc-container-gutter: 40px;">Content</wc-container>'
      );
      const inner = shadowQuery(el, '.container__inner')!;
    ...
    it('--wc-container-max-width overrides max-width on inner', async () => {
      const el = await fixture<WcContainer>(
        '<wc-container style="--wc-container-max-width: 500px;">Content</wc-container>'
      );
      const inner = shadowQuery(el, '.container__inner')!;
      const styles = getComputedStyle(inner);
  ...
  describe('Accessibility', () => {
    ...
    it('has no axe violations with rich content', async () => {
      const el = await fixture<WcContainer>(
        '<wc-container><h2>Section Title</h2><p>Paragraph content here.</p></wc-container>'
      );
      await page.screenshot();
      const { violations } = await checkA11y(el);
      expect(violations).toEqual([]);
