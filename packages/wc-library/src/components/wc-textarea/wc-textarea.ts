...
import { wcTextareaStyles } from './wc-textarea.styles.js';
 ...
 * @fires {CustomEvent<{value: string}>} wc-change - Dispatched when the textarea loses focus after its value changed.
 *
 ...
 * @cssprop [--wc-textarea-min-height=var(--wc-size-20, 5rem)] - Minimum textarea height.
 */
@customElement('wc-textarea')
export class WcTextarea extends LitElement {
  static override styles = [tokenStyles, wcTextareaStyles];
  ...
  private _renderCounter() {
    ...
    const display =
      this.maxlength !== undefined ? `${count} / ${this.maxlength}` : `${count}`;

    return html`
      <div part="counter" class="field__counter">${display}</div>
    `;
  }
      ...
      <div part="field" class=${classMap(fieldClasses)}>
        ...
        <div part="textarea-wrapper" class="field__textarea-wrapper">
          <textarea
            ...
            aria-labelledby=${ifDefined(this._hasLabelSlot ? `${this._textareaId}-slotted-label` : undefined)}
