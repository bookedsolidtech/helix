...
describe('wc-form', () => {
  ...
  describe('Rendering', () => {
    it('renders as Light DOM (no shadowRoot)', async () => {
      const el = await fixture<WcForm>('<wc-form></wc-form>');
      expect(el.shadowRoot).toBeNull();
    });

    it('renders <form> tag when action is set', async () => {
      const el = await fixture<WcForm>('<wc-form action="/submit"></wc-form>');
  ...
  describe('Properties', () => {
    it('action property sets form action attribute', async () => {
      const el = await fixture<WcForm>('<wc-form action="/api/save"></wc-form>');
    ...
    it('method property defaults to post', async () => {
      const el = await fixture<WcForm>('<wc-form action="/api"></wc-form>');
    ...
    it('novalidate property sets novalidate attribute on form', async () => {
      const el = await fixture<WcForm>('<wc-form action="/api" novalidate></wc-form>');
    ...
    it('name property sets name attribute on form', async () => {
      const el = await fixture<WcForm>('<wc-form action="/api" name="login-form"></wc-form>');
  ...
  describe('Events', () => {
    it('dispatches wc-submit on valid client-side submit', async () => {
      ...
      const form = el.querySelector('form')!;
      const eventPromise = oneEvent<CustomEvent>(el, 'wc-submit');
    ...
    it('dispatches wc-reset when form is reset', async () => {
      ...
      const form = el.querySelector('form')!;
      const eventPromise = oneEvent<CustomEvent>(el, 'wc-reset');
  ...
  describe('Form Discovery', () => {
    it('getFormElements() returns wc-* form components', async () => {
      const el = await fixture<WcForm>(`
        <wc-form>
          <wc-text-input name="first" label="First"></wc-text-input>
          <wc-text-input name="last" label="Last"></wc-text-input>
        </wc-form>
      `);

      const elements = el.getFormElements();
      expect(elements.length).toBe(2);
      expect(elements[0].tagName.toLowerCase()).toBe('wc-text-input');
    });
    ...
    it('adopted stylesheet is injected into document', async () => {
      ...
      const hasFormStyles = document.adoptedStyleSheets.some((sheet) => {
        try {
          ...
          return rules.some((rule) => rule.cssText.includes('wc-form'));
        } catch {
          return false;
        }
      });
      ...
      if (formSheet) {
        ...
        for (const rule of nonMediaRules) {
          if (rule instanceof CSSStyleRule) {
            expect(rule.selectorText).toContain('wc-form');
          }
        }
      }
    });
