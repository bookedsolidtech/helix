...
**Source:** `packages/hx-library/src/components/hx-split-button/hx-split-button.ts`
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-split-button.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T11:51:09.195Z
...
| 2.5.5 | Target Size (Enhanced) | Supports | Target ≥44×44 CSS px (essential / equivalent / inline / UA-control exceptions per WCAG 2.2) | target=hx-split-button 173.7×44.0 px | [spec](https://www.w3.org/TR/WCAG22/#target-size-enhanced) |
...
**Measured:** target=hx-split-button 173.7×44.0 px
...
- **Source class:** [`packages/hx-library/src/components/hx-split-button/hx-split-button.ts`](../../../../packages/hx-library/src/components/hx-split-button/hx-split-button.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-split-button/hx-split-button.styles.ts`](../../../../packages/hx-library/src/components/hx-split-button/hx-split-button.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-split-button/hx-split-button.stories.ts`](../../../../packages/hx-library/src/components/hx-split-button/hx-split-button.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-split-button.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-split-button--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-split-button/hx-split-button.ts`) or styles file (`packages/hx-library/src/components/hx-split-button/hx-split-button.styles.ts`) is modified.
