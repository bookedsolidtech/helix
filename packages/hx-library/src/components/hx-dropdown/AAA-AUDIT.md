...
**Source:** `packages/hx-library/src/components/hx-dropdown/hx-dropdown.ts`
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-dropdown.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T20:15:20.174Z
...
| Metric | Value |
|---|---|
| Total `Components/*` stories for this component | 8 |
| Stories with **zero serious findings** (visually confirmed by story-audit harness) | 8 |
| Stories with serious findings (pending re-verification) | 0 |
| Visual-confirmation rate | **100%** |

> All 8 stories for this component cleared the story-audit harness at the serious floor. **Visual confirmation rate: 100%.** The component's AAA cert is supported by both formal-audit measurements (Default story) and pixel-level rendering checks across every shipped variant.
...
| 2.5.5 | Target Size (Enhanced) | Not Applicable | Target ≥44×44 CSS px (essential / equivalent / inline / UA-control exceptions per WCAG 2.2) | target=hx-dropdown 139.4×44.0 px | [spec](https://www.w3.org/TR/WCAG22/#target-size-enhanced) |
...
**Evidence:** Component target (hx-dropdown) is transparent (bg=rgba(0, 0, 0, 0)); foreground contrast inherits the consumer page background. No component-level contrast obligation. Manual: verify that documented surface-token combinations meet 7:1.
...
**Measured:** target=hx-dropdown 139.4×44.0 px
...
- **Source class:** [`packages/hx-library/src/components/hx-dropdown/hx-dropdown.ts`](../../../../packages/hx-library/src/components/hx-dropdown/hx-dropdown.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-dropdown/hx-dropdown.styles.ts`](../../../../packages/hx-library/src/components/hx-dropdown/hx-dropdown.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-dropdown/hx-dropdown.stories.ts`](../../../../packages/hx-library/src/components/hx-dropdown/hx-dropdown.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-dropdown.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-dropdown--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-dropdown/hx-dropdown.ts`) or styles file (`packages/hx-library/src/components/hx-dropdown/hx-dropdown.styles.ts`) is modified.
