...
The WAI-ARIA spec states: *"If a menu is visually persistent, it is a menubar. Otherwise, it is best to open menus and submenus as pop-ups."* The spec also requires all `menu` roles to have an accessible name via `aria-label` or `aria-labelledby`.
