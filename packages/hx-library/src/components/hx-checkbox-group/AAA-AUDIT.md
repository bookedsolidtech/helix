...
**Source:** `packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.ts`
**ARIA pattern:** [`group`](https://www.w3.org/TR/wai-aria-1.2/#group) (WAI-ARIA APG)
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-checkbox-group.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T20:15:07.804Z
...
**Headline:** **Supports + Not Applicable across all 11 criteria** (6 Supports, 5 Not Applicable). The component meets WCAG 2.2 AAA on every applicable Success Criterion and conforms to both peer standards (forced-colors, APG keyboard contract).
...
| Metric | Value |
|---|---|
| Total `Components/*` stories for this component | 21 |
| Stories with **zero serious findings** (visually confirmed by story-audit harness) | 21 |
| Stories with serious findings (pending re-verification) | 0 |
| Visual-confirmation rate | **100%** |

> All 21 stories for this component cleared the story-audit harness at the serious floor. **Visual confirmation rate: 100%.** The component's AAA cert is supported by both formal-audit measurements (Default story) and pixel-level rendering checks across every shipped variant.
...
| apg-keyboard | APG-Aligned Keyboard Contract | Supports | Keyboard interaction matches WAI-ARIA APG pattern for declared `@aria-pattern` | @aria-pattern="group" requires no specific key set (alert / textbox). | [spec](https://www.w3.org/WAI/ARIA/apg/patterns/) |
...
**Measured:** @aria-pattern="group" requires no specific key set (alert / textbox).

**Evidence:** @aria-pattern="group" requires no specific key set (alert / textbox).

**APG pattern reference:** [https://www.w3.org/TR/wai-aria-1.2/#group](https://www.w3.org/TR/wai-aria-1.2/#group)

**Pattern note (from aaa-standards.json):** Tier-1 Task-5 (2026-05-08): role=group from WAI-ARIA 1.2 — a structural grouping role with no required keyboard contract; items inside the group dictate keyboard behaviour.
...
- **Source class:** [`packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.ts`](../../../../packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.styles.ts`](../../../../packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.stories.ts`](../../../../packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-checkbox-group.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-checkbox-group--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.ts`) or styles file (`packages/hx-library/src/components/hx-checkbox-group/hx-checkbox-group.styles.ts`) is modified.
