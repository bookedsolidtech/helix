...
**Component:** `hx-switch`
**Source:** `packages/hx-library/src/components/hx-switch/hx-switch.ts`
**ARIA pattern:** [`switch`](https://www.w3.org/WAI/ARIA/apg/patterns/switch/) (WAI-ARIA APG)
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-switch.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T19:46:37.031Z
...
| Metric | Value |
|---|---|
| Total `Components/*` stories for this component | 31 |
| Stories with **zero serious findings** (visually confirmed by story-audit harness) | 31 |
| Stories with serious findings (pending re-verification) | 0 |
| Visual-confirmation rate | **100%** |

> All 31 stories for this component cleared the story-audit harness at the serious floor. **Visual confirmation rate: 100%.** The component's AAA cert is supported by both formal-audit measurements (Default story) and pixel-level rendering checks across every shipped variant.
...
| 2.5.5 | Target Size (Enhanced) | Supports | Target ≥44×44 CSS px (essential / equivalent / inline / UA-control exceptions per WCAG 2.2) | target=hx-switch 1184.0×44.0 px | [spec](https://www.w3.org/TR/WCAG22/#target-size-enhanced) |
...
| apg-keyboard | APG-Aligned Keyboard Contract | Supports | Keyboard interaction matches WAI-ARIA APG pattern for declared `@aria-pattern` | @aria-pattern="switch" — all required key groups referenced (' '\|Enter). | [spec](https://www.w3.org/WAI/ARIA/apg/patterns/) |
...
**Measured:** target=hx-switch 1184.0×44.0 px
...
**Measured:** @aria-pattern="switch" — all required key groups referenced (' '|Enter).

**Evidence:** @aria-pattern="switch" — all required key groups referenced (' '|Enter).

**APG pattern reference:** [https://www.w3.org/WAI/ARIA/apg/patterns/switch/](https://www.w3.org/WAI/ARIA/apg/patterns/switch/)
...
- **Source class:** [`packages/hx-library/src/components/hx-switch/hx-switch.ts`](../../../../packages/hx-library/src/components/hx-switch/hx-switch.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-switch/hx-switch.styles.ts`](../../../../packages/hx-library/src/components/hx-switch/hx-switch.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-switch/hx-switch.stories.ts`](../../../../packages/hx-library/src/components/hx-switch/hx-switch.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-switch.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-switch--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-switch/hx-switch.ts`) or styles file (`packages/hx-library/src/components/hx-switch/hx-switch.styles.ts`) is modified.
