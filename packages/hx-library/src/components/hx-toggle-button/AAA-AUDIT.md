...
**Source:** `packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.ts`
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-toggle-button.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T19:46:46.208Z
...
| 2.5.5 | Target Size (Enhanced) | Supports | Target ≥44×44 CSS px (essential / equivalent / inline / UA-control exceptions per WCAG 2.2) | target=hx-toggle-button 158.4×44.0 px | [spec](https://www.w3.org/TR/WCAG22/#target-size-enhanced) |
...
| 3.3.6 | Error Prevention (All) | Supports | Submission is reversible, checked, or confirmable | Form-associated submission trigger that also exposes ElementInternals + setValidity for any stateful value (e.g. toggle pressed state). | [spec](https://www.w3.org/TR/WCAG22/#error-prevention-all) |
...
**Measured:** target=hx-toggle-button 158.4×44.0 px
...
**Measured:** Form-associated submission trigger that also exposes ElementInternals + setValidity for any stateful value (e.g. toggle pressed state).

**Evidence:** Form-associated submission trigger that also exposes ElementInternals + setValidity for any stateful value (e.g. toggle pressed state).
...
- **Source class:** [`packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.ts`](../../../../packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.styles.ts`](../../../../packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.stories.ts`](../../../../packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-toggle-button.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-togglebutton--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.ts`) or styles file (`packages/hx-library/src/components/hx-toggle-button/hx-toggle-button.styles.ts`) is modified.
