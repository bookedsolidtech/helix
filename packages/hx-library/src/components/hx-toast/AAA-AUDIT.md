...
**Component:** `hx-toast`
**Source:** `packages/hx-library/src/components/hx-toast/hx-toast.ts`
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-toast.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T20:15:58.490Z
...
| Metric | Value |
|---|---|
| Total `Components/*` stories for this component | 17 |
| Stories with **zero serious findings** (visually confirmed by story-audit harness) | 17 |
| Stories with serious findings (pending re-verification) | 0 |
| Visual-confirmation rate | **100%** |

> All 17 stories for this component cleared the story-audit harness at the serious floor. **Visual confirmation rate: 100%.** The component's AAA cert is supported by both formal-audit measurements (Default story) and pixel-level rendering checks across every shipped variant.
...
**Evidence:** Component target (hx-toast) is transparent (bg=rgba(0, 0, 0, 0)); foreground contrast inherits the consumer page background. No component-level contrast obligation. Manual: verify that documented surface-token combinations meet 7:1.
...
- **Source class:** [`packages/hx-library/src/components/hx-toast/hx-toast.ts`](../../../../packages/hx-library/src/components/hx-toast/hx-toast.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-toast/hx-toast.styles.ts`](../../../../packages/hx-library/src/components/hx-toast/hx-toast.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-toast/hx-toast.stories.ts`](../../../../packages/hx-library/src/components/hx-toast/hx-toast.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-toast.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-toast--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-toast/hx-toast.ts`) or styles file (`packages/hx-library/src/components/hx-toast/hx-toast.styles.ts`) is modified.
