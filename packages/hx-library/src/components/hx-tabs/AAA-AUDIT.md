...
**Component:** `hx-tabs`
**Source:** `packages/hx-library/src/components/hx-tabs/hx-tabs.ts`
**ARIA pattern:** [`tabs`](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/) (WAI-ARIA APG)
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-tabs.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T20:15:52.128Z
...
| 2.5.5 | Target Size (Enhanced) | Supports | Target ≥44×44 CSS px (essential / equivalent / inline / UA-control exceptions per WCAG 2.2) | target=hx-tabs 1184.0×133.0 px | [spec](https://www.w3.org/TR/WCAG22/#target-size-enhanced) |
...
| apg-keyboard | APG-Aligned Keyboard Contract | Supports | Keyboard interaction matches WAI-ARIA APG pattern for declared `@aria-pattern` | @aria-pattern="tabs" — all required key groups referenced (ArrowLeft\|ArrowRight\|ArrowUp\|ArrowDown, Home, End). | [spec](https://www.w3.org/WAI/ARIA/apg/patterns/) |
...
**Evidence:** Component target (hx-tabs) is transparent (bg=rgba(0, 0, 0, 0)); foreground contrast inherits the consumer page background. No component-level contrast obligation. Manual: verify that documented surface-token combinations meet 7:1.
...
**Measured:** target=hx-tabs 1184.0×133.0 px
...
**Measured:** @aria-pattern="tabs" — all required key groups referenced (ArrowLeft|ArrowRight|ArrowUp|ArrowDown, Home, End).

**Evidence:** @aria-pattern="tabs" — all required key groups referenced (ArrowLeft|ArrowRight|ArrowUp|ArrowDown, Home, End).

**APG pattern reference:** [https://www.w3.org/WAI/ARIA/apg/patterns/tabs/](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/)
...
- **Source class:** [`packages/hx-library/src/components/hx-tabs/hx-tabs.ts`](../../../../packages/hx-library/src/components/hx-tabs/hx-tabs.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-tabs/hx-tabs.styles.ts`](../../../../packages/hx-library/src/components/hx-tabs/hx-tabs.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-tabs/hx-tabs.stories.ts`](../../../../packages/hx-library/src/components/hx-tabs/hx-tabs.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-tabs.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-tabs--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-tabs/hx-tabs.ts`) or styles file (`packages/hx-library/src/components/hx-tabs/hx-tabs.styles.ts`) is modified.
