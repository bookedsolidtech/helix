...
**Component:** `hx-rating`
**Source:** `packages/hx-library/src/components/hx-rating/hx-rating.ts`
**ARIA pattern:** [`radiogroup`](https://www.w3.org/WAI/ARIA/apg/patterns/radio/) (WAI-ARIA APG)
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-rating.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T19:46:29.489Z
...
| 2.5.5 | Target Size (Enhanced) | Supports | Target ≥44×44 CSS px (essential / equivalent / inline / UA-control exceptions per WCAG 2.2) | target=hx-rating 236.0×44.0 px | [spec](https://www.w3.org/TR/WCAG22/#target-size-enhanced) |
...
| apg-keyboard | APG-Aligned Keyboard Contract | Supports | Keyboard interaction matches WAI-ARIA APG pattern for declared `@aria-pattern` | @aria-pattern="radiogroup" — all required key groups referenced (ArrowDown\|ArrowUp, ArrowLeft\|ArrowRight). | [spec](https://www.w3.org/WAI/ARIA/apg/patterns/) |
...
**Evidence:** Component target (hx-rating) is transparent (bg=rgba(0, 0, 0, 0)); foreground contrast inherits the consumer page background. No component-level contrast obligation. Manual: verify that documented surface-token combinations meet 7:1.
...
**Measured:** target=hx-rating 236.0×44.0 px
...
**Measured:** @aria-pattern="radiogroup" — all required key groups referenced (ArrowDown|ArrowUp, ArrowLeft|ArrowRight).

**Evidence:** @aria-pattern="radiogroup" — all required key groups referenced (ArrowDown|ArrowUp, ArrowLeft|ArrowRight).

**APG pattern reference:** [https://www.w3.org/WAI/ARIA/apg/patterns/radio/](https://www.w3.org/WAI/ARIA/apg/patterns/radio/)
...
- **Source class:** [`packages/hx-library/src/components/hx-rating/hx-rating.ts`](../../../../packages/hx-library/src/components/hx-rating/hx-rating.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-rating/hx-rating.styles.ts`](../../../../packages/hx-library/src/components/hx-rating/hx-rating.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-rating/hx-rating.stories.ts`](../../../../packages/hx-library/src/components/hx-rating/hx-rating.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-rating.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-rating--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-rating/hx-rating.ts`) or styles file (`packages/hx-library/src/components/hx-rating/hx-rating.styles.ts`) is modified.
