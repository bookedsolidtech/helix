...
**Source:** `packages/hx-library/src/components/hx-radio-group/hx-radio-group.ts`
...
**Audit method:** Automated browser harness `scripts/aaa-formal-audit.mjs` driving the component's **Default story** in Storybook (Chromium via Playwright). Per-criterion measurements (computed contrast ratios, target sizes, focus-appearance pixel measurements, axe-core rule outcomes, keyboard-event traces) written to `.reports/formal-aaa-audit/evidence/hx-radio-group.json`. The audit does NOT exercise every story / variant / state of the component — see "Variant coverage" for the full picture.
**Audit run:** 2026-05-10T11:51:01.847Z
...
| Metric | Value |
|---|---|
| Total `Components/*` stories for this component | 29 |
| Stories with **zero serious findings** (visually confirmed by story-audit harness) | 29 |
| Stories with serious findings (pending re-verification) | 0 |
| Visual-confirmation rate | **100%** |

> All 29 stories for this component cleared the story-audit harness at the serious floor. **Visual confirmation rate: 100%.** The component's AAA cert is supported by both formal-audit measurements (Default story) and pixel-level rendering checks across every shipped variant.
...
| 2.5.5 | Target Size (Enhanced) | Supports | Target ≥44×44 CSS px (essential / equivalent / inline / UA-control exceptions per WCAG 2.2) | target=hx-radio-group 1184.0×263.0 px | [spec](https://www.w3.org/TR/WCAG22/#target-size-enhanced) |
...
**Measured:** target=hx-radio-group 1184.0×263.0 px
...
- **Source class:** [`packages/hx-library/src/components/hx-radio-group/hx-radio-group.ts`](../../../../packages/hx-library/src/components/hx-radio-group/hx-radio-group.ts)
- **Source styles:** [`packages/hx-library/src/components/hx-radio-group/hx-radio-group.styles.ts`](../../../../packages/hx-library/src/components/hx-radio-group/hx-radio-group.styles.ts)
- **Stories:** [`packages/hx-library/src/components/hx-radio-group/hx-radio-group.stories.ts`](../../../../packages/hx-library/src/components/hx-radio-group/hx-radio-group.stories.ts)
- **Per-cell evidence (gitignored):** `.reports/formal-aaa-audit/evidence/hx-radio-group.json`
- **Aggregate matrix (gitignored):** `.reports/formal-aaa-audit/audit.matrix.md`
- **Storybook URL audited:** `http://localhost:3151/iframe.html?id=components-radio-group--default&viewMode=story`
...
- The component's class file (`packages/hx-library/src/components/hx-radio-group/hx-radio-group.ts`) or styles file (`packages/hx-library/src/components/hx-radio-group/hx-radio-group.styles.ts`) is modified.
