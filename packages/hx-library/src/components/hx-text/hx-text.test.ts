...
import type { HelixText } from './hx-text.js';
import './index.js';
...
describe('hx-text', () => {
  ...
  describe('Rendering', () => {
    it('renders with shadow DOM', async () => {
      const el = await fixture<HelixText>('<hx-text>Hello</hx-text>');
    ...
    it('renders default slot text content', async () => {
      const el = await fixture<HelixText>('<hx-text>Patient record</hx-text>');
      expect(el.textContent?.trim()).toBe('Patient record');
    });

    it('applies both variant and color classes simultaneously', async () => {
      const el = await fixture<HelixText>(
        '<hx-text variant="label" color="danger">Alert</hx-text>',
      );
      const base = shadowQuery(el, '[part="base"]')!;
  ...
  describe('Property: variant', () => {
    ...
    it('reflects variant attribute to host', async () => {
      const el = await fixture<HelixText>('<hx-text variant="label">Text</hx-text>');
      expect(el.getAttribute('variant')).toBe('label');
    });

    for (const variant of variants) {
      it(`applies text--${variant} class for variant="${variant}"`, async () => {
        const el = await fixture<HelixText>(`<hx-text variant="${variant}">Text</hx-text>`);
        const base = shadowQuery(el, '[part="base"]')!;
  ...
  describe('Property: weight', () => {
    ...
    for (const weight of weights) {
      it(`applies text--weight-${weight} class for weight="${weight}"`, async () => {
        const el = await fixture<HelixText>(`<hx-text weight="${weight}">Text</hx-text>`);
        const base = shadowQuery(el, '[part="base"]')!;
  ...
  describe('Property: color', () => {
    ...
    it('reflects color attribute to host', async () => {
      const el = await fixture<HelixText>('<hx-text color="subtle">Text</hx-text>');
      expect(el.getAttribute('color')).toBe('subtle');
    });

    for (const color of colors) {
      it(`applies text--color-${color} class for color="${color}"`, async () => {
        const el = await fixture<HelixText>(`<hx-text color="${color}">Text</hx-text>`);
        const base = shadowQuery(el, '[part="base"]')!;
  ...
  describe('Property: as', () => {
    ...
    it('renders a <p> base when as="p"', async () => {
      const el = await fixture<HelixText>('<hx-text as="p">Text</hx-text>');
    ...
    it('falls back to span for unknown as value', async () => {
      const el = await fixture<HelixText>('<hx-text as="script">Text</hx-text>');
      const base = shadowQuery(el, 'span[part="base"]');
      expect(base).toBeInstanceOf(HTMLSpanElement);
    });
    ...
    it('sanitizes dangerous tag like "img" to span', async () => {
      const el = await fixture<HelixText>('<hx-text as="img">Text</hx-text>');
      const base = shadowQuery(el, 'span[part="base"]');
      expect(base).toBeInstanceOf(HTMLSpanElement);
    });
  });
  ...
  describe('Property: lines', () => {
    ...
    it('does not apply text--truncate when lines > 0', async () => {
      const el = await fixture<HelixText>('<hx-text lines="2" truncate>Text</hx-text>');
      const base = shadowQuery(el, '[part="base"]')!;
      expect(base.classList.contains('text--truncate')).toBe(false);
    });
    ...
    it('treats negative lines as 0 (no clamping)', async () => {
      const el = await fixture<HelixText>('<hx-text lines="-1">Text</hx-text>');
      const base = shadowQuery(el, '[part="base"]')!;
      expect(base.classList.contains('text--clamp')).toBe(false);
      expect(base.style.getPropertyValue('-webkit-line-clamp')).toBe('');
    });

    it('lines overrides truncate when both set', async () => {
      const el = await fixture<HelixText>('<hx-text lines="2" truncate>Text</hx-text>');
      const base = shadowQuery(el, '[part="base"]')!;
      expect(base.classList.contains('text--clamp')).toBe(true);
      expect(base.classList.contains('text--truncate')).toBe(false);
      expect(base.style.getPropertyValue('-webkit-line-clamp')).toBe('2');
    });
  });
  ...
  describe('Truncation: title attribute', () => {
    it('adds title to base span when truncate=true', async () => {
      const el = await fixture<HelixText>('<hx-text truncate>Full patient name here</hx-text>');
      const base = shadowQuery(el, '[part="base"]')!;
      expect(base.getAttribute('title')).toBe('Full patient name here');
    });

    it('adds title to base span when lines > 0', async () => {
      const el = await fixture<HelixText>('<hx-text lines="2">Long clinical note text</hx-text>');
      const base = shadowQuery(el, '[part="base"]')!;
      expect(base.getAttribute('title')).toBe('Long clinical note text');
    });

    it('does not add title when text is not truncated', async () => {
      const el = await fixture<HelixText>('<hx-text>Normal text</hx-text>');
      const base = shadowQuery(el, '[part="base"]')!;
  ...
  describe('Accessibility (axe-core)', () => {
    it('has no axe violations in default state', async () => {
      const el = await fixture<HelixText>('<hx-text>Hello patient</hx-text>');
    ...
    it('has no axe violations for all variants', async () => {
      ...
      for (const variant of variants) {
        const el = await fixture<HelixText>(`<hx-text variant="${variant}">Sample text</hx-text>`);
        await page.screenshot();
        const { violations } = await checkA11y(el);
    ...
    it('has no axe violations for all colors', async () => {
      ...
      for (const color of colors) {
        const el = await fixture<HelixText>(`<hx-text color="${color}">Sample text</hx-text>`);
        await page.screenshot();
        const { violations } = await checkA11y(el);
    ...
    it('has no axe violations when truncated', async () => {
      const el = await fixture<HelixText>(
        '<hx-text truncate>Long text that gets clipped</hx-text>',
      );
      await page.screenshot();
      const { violations } = await checkA11y(el);
      expect(violations).toEqual([]);
