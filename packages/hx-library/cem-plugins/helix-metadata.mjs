...
// ── AAA verdicts index (audit-derived `helixMeta.aaa.criteria`) ────────────

/**
 * Build a tagName → criteria-list reverse index from the committed AAA
 * verdicts snapshot at `packages/hx-library/aaa-verdicts.json`. The snapshot
 * is generated by `scripts/generate-aaa-verdicts.mjs` after every
 * `pnpm aaa:audit` run. When present, the index is the authoritative source
 * for `helixMeta.aaa.criteria` — the per-component `@aaa-criteria` JSDoc
 * tag becomes a fallback for components missing from the snapshot (e.g.
 * components added between audit runs).
 *
 * Returns an empty Map when the snapshot is missing or malformed so the
 * plugin still emits all other metadata cleanly.
 */
function loadAaaVerdictsIndex() {
  const index = new Map();
  let verdictsPath;
  try {
    const here = dirname(fileURLToPath(import.meta.url));
    verdictsPath = resolve(here, '..', 'aaa-verdicts.json');
  } catch (err) {
    console.warn(`[helix-metadata] could not resolve aaa-verdicts.json path: ${err.message}`);
    return index;
  }

  if (!existsSync(verdictsPath)) {
    // Snapshot not generated yet — silent fallback to JSDoc tags. This is the
    // expected state on a fresh clone before `pnpm aaa:audit` runs once.
    return index;
  }
  ...
  try {
    ...
    console.warn(
      `[helix-metadata] aaa-verdicts.json failed to parse (${err.message}); ` +
        `AAA criteria will fall back to @aaa-criteria JSDoc tags.`,
    );
    return index;
  }
  ...
  for (const [tag, verdicts] of Object.entries(components)) {
    if (typeof tag !== 'string' || !verdicts || typeof verdicts !== 'object') continue;
    const supports = [];
    for (const [criterion, entry] of Object.entries(verdicts)) {
      if (entry?.verdict === 'Supports') supports.push(criterion);
    }
    if (supports.length > 0) index.set(tag, supports);
  }
  return index;
}
...
/**
 * Plugin entry point. Pass `{ priorityTierIndex, aaaVerdictsIndex }` to inject
 * pre-built indexes (used by the unit tests); otherwise the plugin loads them
 * from disk on first call.
 */
export function helixMetadataPlugin(options = {}) {
  let priorityTierIndex = options.priorityTierIndex;
  let aaaVerdictsIndex = options.aaaVerdictsIndex;
  ...
  return {
    ...
    analyzePhase({ ts, node, moduleDoc }) {
      ...
      // Lazy-load the indexes on first hit so the plugin is cheap to construct
      // in tests that pass their own indexes.
      if (priorityTierIndex === undefined) {
        priorityTierIndex = loadPriorityTierIndex();
      }
      if (aaaVerdictsIndex === undefined) {
        aaaVerdictsIndex = loadAaaVerdictsIndex();
      }
      ...
      // Audit-derived override: when the AAA verdicts snapshot lists this
      // component, replace the hand-decorated criteria with the harness's
      // measured Supports list. The snapshot is the single source of truth
      // (per the 3.9.0 self-cert remediation); the @aaa-criteria JSDoc tag is
      // a fallback for components not yet in the snapshot.
      if (
        helixMeta &&
        classDoc.tagName &&
        aaaVerdictsIndex.has(classDoc.tagName) &&
        helixMeta.aaa
      ) {
        helixMeta.aaa.criteria = aaaVerdictsIndex.get(classDoc.tagName);
      }
