...
- c0357eb: fix leaked JavaScript in the distributed `dist/css/` bundles for components whose `.styles.ts` exports multiple `css`` literals
  ...
  - `css:validate` gains a content gate that scans every `dist/css/*.css` file for leaked-JavaScript signatures (`css`` starts, `export`/`import`lines, arrow functions,`${`interpolations, tagged-literal terminators) and fails with`file:line`on any hit. It runs at the end of`build`, and a new `prepack`lifecycle script rebuilds the package before packing, so`npm pack`/publish regenerates the JS, CEM, and CSS artifacts together from source and refuses a leaking tarball rather than shipping one.
