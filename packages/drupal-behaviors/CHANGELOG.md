...
- e4b79be: BREAKING: Drupal behaviors re-aligned with the `@helixui/library@3.0.0` component releases. Consumers attaching to these behaviors should verify their integrations against the updated surface.

  Changes:
  - Behaviors now consume the form-control state and validity helpers exposed by `FormMixin` (state/validity helpers, not a dedicated event channel — components still dispatch their own `hx-change` / `hx-input` events per CEM)
  - Component tag references corrected to match shipped element names (see commit `aef35b4c7`)
  - Behaviors continue to read the library's documented label APIs (`aria-label` on non-interactive surfaces, component-specific `hx-label` where the CEM exposes it)
...
- de9ccbe: fix(hx-menu): repair drupal behavior hx-close integration, add max-height overflow scroll, add arrowleft submenu close event
  - Rewrite the standalone `src/behaviors/hx-menu.behavior.js` to listen for the `hx-close`
    event dispatched by hx-menu instead of the no-op `menu.open = false` setter. Removes the
    redundant Escape keydown listener (hx-menu already fires hx-close on Escape). Adds optional
    trigger button `aria-expanded` toggle and focus-return on close.
    **Scope:** the rewrite landed only in the per-behavior file. The combined `src/index.js`
    bundle still ships the older `menu.open = false` path — consumers loading individual
    behavior files get the fix, consumers loading the bundle do not. Tracked for a follow-up
    sync in `packages/drupal-behaviors/src/index.js`.
