...
### Package Manager & Task Runner: npm + Turborepo

**Decision**: npm (workspaces) + Turborepo over pnpm/yarn.

**Rationale**: npm is the standard Node.js package manager with zero additional installation requirements -- it ships with every Node.js distribution. npm workspaces (available since npm 7) provide native workspace linking and hoisting. Turborepo adds intelligent task orchestration with caching, parallel execution, and dependency-aware build pipelines on top of npm workspaces. This combination provides enterprise reliability with minimal toolchain complexity.
...
+-- package.json                      # Root scripts, shared devDependencies, workspaces
