# 4. Documentation Hub Architecture

> **Section Owner**: Principal Engineer
> **Last Updated**: 2026-02-13
> **Status**: Draft for tech lead interview preparation (2026-02-17)
    ...
    async load({ store, logger }) {
      ...
      for (const module of manifest.modules ?? []) {
        for (const declaration of module.declarations ?? []) {
          ...
          // e.g., src/components/atoms/chc-button/chc-button.ts -> atoms
...
for (const module of manifest.modules ?? []) {
  for (const declaration of module.declarations ?? []) {
    ...
    // Human-friendly component name: chc-content-card -> Content Card
    ...
    writeFileSync(join(categoryDir, `${tagName.replace('chc-', '')}.mdx`), content);
  }
}
...
```
Developer edits chc-button.ts JSDoc
        |
        v
...
```bash
# Build pipeline
turbo run build --filter=storybook                 # Builds static Storybook
cp -r apps/storybook/storybook-static apps/docs/public/storybook  # Copy to docs
turbo run build --filter=docs                      # Builds Starlight (includes Storybook)
```
...
The `<chc-content-card>` component maps directly to Drupal's article node
teaser view mode.
...
@customElement('chc-button')  // highlighted
export class ChcButton extends LitElement {  // highlighted
...
````markdown
```typescript title="chc-button.ts"
...
```
[1. Build Library]
    turbo run build --filter=@org/wc-library
    |
...
jobs:
  build-docs:
    ...
    steps:
      ...
      - run: turbo run build --filter=@org/wc-library
      - run: turbo run cem --filter=@org/wc-library

      # 2. Build Storybook
      - run: turbo run build --filter=storybook
...
The docs app is added to the existing monorepo workspace (defined in the root `package.json`). Task orchestration is handled by `turbo.json`:
...
{
  "scripts": {
    "build": "turbo run build --filter=@org/wc-library",
    "build:tokens": "node tools/tokens/build-tokens.mjs",
    "build:cem": "turbo run cem --filter=@org/wc-library",
    "dev": "turbo run dev --filter=storybook",
    "dev:docs": "turbo run dev --filter=docs",
    "test": "turbo run test --filter=@org/wc-library",
    "lint": "turbo run lint",
    ...
    "docs:build": "turbo run build build:cem && turbo run build --filter=storybook && turbo run build --filter=docs",
