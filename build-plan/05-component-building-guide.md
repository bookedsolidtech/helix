...
- Front-end developers building Lit Web Components for the CHC (Content Hub Components) library
...
- `chc-` prefix on all custom element tag names (Content Hub Components)
- `--chc-` prefix on all CSS custom properties
...
1. Prefix all events with `chc-` to avoid collision with native DOM events
2. Use kebab-case: `chc-card-click`, `chc-form-submit`, `chc-nav-toggle`
...
<chc-content-card heading="Understanding Anxiety" href="/articles/understanding-anxiety">
...
Before the component's JavaScript loads, the browser renders `<chc-content-card>` as an unknown element -- a block-level box with its children visible in the light DOM. The image, paragraph, and link are all visible and functional.
...
chc-content-card:not(:defined) {
  display: block;
  border: 1px solid var(--chc-color-border, #e2e8f0);
  border-radius: 8px;
  padding: 1rem;
  overflow: hidden;
}
...
chc-content-card:not(:defined) .js-only {
  display: none;
}
```
...
<chc-nav items='{{ menu_items_json }}'></chc-nav>
```
...
<chc-content-card featured="false">
```
...
```twig
<chc-content-card variant="{{ view_mode == 'teaser_featured' ? 'featured' : 'default' }}">
```
...
### 5.4.1 Content Card (`chc-content-card`)
...
{#
  CHC Content Card Integration
  =============================
  Maps Drupal article node fields to the chc-content-card web component.
...
### 5.4.2 Article Header (`chc-article-header`)
...
### 5.4.3 Media Component (`chc-media`)
...
{#
  CHC Media Component Integration
  ================================
  Wraps Drupal's media image field output in the chc-media component.
...
### 5.4.4 Text Input (`chc-text-input`)
...
### 5.4.5 Textarea (`chc-textarea`)

**Purpose**: Multi-line text input for healthcare forms. Supports character counting, auto-resize, and the same validation patterns as `chc-text-input`.
...
### 5.4.8 Radio Group (`chc-radio-group`)
...
{#
  CHC Navigation Integration
  ===========================
  Serializes Drupal's menu tree into the JSON format expected by chc-nav.
...
>
  ...
  <div slot="actions">
    <chc-button variant="ghost" aria-label="{{ 'Search'|t }}">
      <chc-icon name="search"></chc-icon>
    </chc-button>
    {% if logged_in %}
      <chc-button variant="secondary" href="{{ path('user.page') }}">
        {{ 'My Account'|t }}
      </chc-button>
    {% else %}
      <chc-button variant="primary" href="{{ path('user.login') }}">
        {{ 'Patient Login'|t }}
      </chc-button>
    {% endif %}
  </div>
 ...
 *
 * Serializes the menu tree into JSON for the chc-nav component.
 */
...
### 5.4.10 Hero Banner (`chc-hero-banner`)
...
{#
  CHC Hero Banner Integration
  ============================
  Maps a Drupal "Hero" paragraph type to the chc-hero-banner component.
...
### 5.4.11 Accordion (`chc-accordion`)
...
{#
  CHC Accordion Integration
  ==========================
  Maps a Drupal "FAQ" paragraph type to the chc-accordion component.
...
<chc-accordion heading-level="3" {{ attributes }}>
...
### 5.4.12 Alert / Notification (`chc-alert`)
...
- [ ] All custom events use the `chc-` prefix
...
- [ ] All customizable properties use the `--chc-` prefix
...
<chc-content-card heading="My Article" author-name="Dr. Chen">
    ...
    <div class="card__media">
      <slot name="media">
        <div class="card__media-placeholder" aria-hidden="true">
          <chc-icon name="image" size="48"></chc-icon>
        </div>
      </slot>
    </div>
  `;
...
```typescript
/**
 * @cssprop [--chc-card-bg=var(--chc-color-surface)] - Card background color
 * @cssprop [--chc-card-padding=var(--chc-spacing-lg)] - Card internal padding
 */
```
...
<chc-content-card heading="Article Title"></chc-content-card>
```
...
<chc-content-card heading="Article Title" href="/articles/my-article">
...
4. chc-components.js loads (ES module, type="module")
...
Drupal.behaviors.chcCardAnalytics = {
  attach(context) {
    ...
    cards.forEach((card) => {
      card.addEventListener('chc-card-click', handleCardClick);
    });
  },
  detach(context, settings, trigger) {
    if (trigger === 'unload') {
      const cards = once.remove('chc-card-analytics', 'chc-content-card', context);
      cards.forEach((card) => {
        card.removeEventListener('chc-card-click', handleCardClick);
      });
    }
  },
};
...
--chc-color-primary
--chc-color-surface
--chc-color-on-surface
--chc-color-border
--chc-spacing-sm, --chc-spacing-md, --chc-spacing-lg
--chc-radius-sm, --chc-radius-md, --chc-radius-lg
--chc-shadow-sm, --chc-shadow-md
--chc-font-size-base, --chc-font-size-lg
```
...
--chc-card-bg: var(--chc-color-surface);
--chc-card-padding: var(--chc-spacing-lg);
--chc-card-radius: var(--chc-radius-md);
--chc-card-shadow: var(--chc-shadow-sm);
--chc-card-border-color: var(--chc-color-border);
```
...
:host {
  /*
   * Resolution order:
   * 1. Component token: --chc-card-bg (most specific, set by consumer)
   * 2. Global token: --chc-color-surface (theme-level)
   * 3. Hardcoded fallback: #ffffff (last resort, no tokens loaded)
   */
  background: var(--chc-card-bg, var(--chc-color-surface, #ffffff));
}
```
...
export class ChcProse extends LitElement {
  ...
  static styles = css`
    ...
    :host {
      display: block;
      max-width: var(--chc-prose-max-width, 720px);
      margin: 0 auto;
    }
  `;
...
EVENTS
  - chc- prefix                          chc-card-click, chc-form-submit
...
CSS CUSTOM PROPERTIES
  - --chc-[component]-[property]         --chc-card-bg, --chc-card-radius
  - Three-level fallback chain           var(--chc-card-bg, var(--chc-color-surface, #fff))
