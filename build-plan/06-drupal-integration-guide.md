...
All components use the `chc-` prefix (Content Hub Components). Every HTML tag starts with `chc-`:

```html
<chc-content-card>
  <chc-button>
    <chc-hero-banner> <chc-text-input></chc-text-input></chc-hero-banner></chc-button
></chc-content-card>
```
...
- Custom elements are registered: `document.querySelector('chc-content-card')` returns an element (if one exists on the page)
...
```yaml
# chc_components.info.yml
name: 'CHC Web Components'
type: module
description: 'Provides the Healthcare Content Hub Web Component library'
package: 'CHC'
core_version_requirement: ^10.3 || ^11
```
 ...
 */
function chc_components_page_attachments(array &$attachments): void {
  $config = \Drupal::config('chc_components.settings');

  if ($config->get('load_globally')) {
    $attachments['#attached']['library'][] = 'chc_components/chc-components';
  }
}
 ...
 */
function chc_components_library_info_alter(array &$libraries, string $extension): void {
  if ($extension !== 'chc_components') {
    return;
  }
  ...
  if ($cdn_url && isset($libraries['chc-components'])) {
    // Replace local paths with CDN URLs
    $version = $config->get('version') ?? '1.0.0';
    $libraries['chc-components']['js'] = [
...
```twig
{# Basic button #}
<chc-button variant="primary">
  Schedule Appointment
</chc-button>

{# Badge with dynamic content #}
<chc-badge variant="success">
  {{ 'Published'|t }}
</chc-badge>

{# Icon with attribute #}
<chc-icon name="calendar" size="24"></chc-icon>
```
...
```twig
<chc-article-layout has-sidebar>
...
{% if node.field_hero_image.entity %}
  ...
  >
    {% if node.field_cta_text.value %}
      <chc-button slot="cta" variant="primary" href="{{ node.field_cta_url.0.url }}">
...
{% if node.field_related_articles|length > 0 %}
  <chc-card-grid columns="3" gap="lg">
...
function mytheme_preprocess_node__article__teaser(array &$variables): void {
  ...
  if (!$node->get('field_media')->isEmpty()) {
    ...
    if ($media && $media->hasField('field_media_image')) {
      ...
      if ($image) {
        $variables['chc_card']['image_url'] = \Drupal::service('file_url_generator')
          ->generateAbsoluteString($image->getFileUri());
        $variables['chc_card']['image_alt'] = $media->get('field_media_image')->alt ?? '';
      }
    }
  }
}
...
<chc-content-card
  heading="{{ chc_card.heading }}"
  summary="{{ chc_card.summary }}"
  category="{{ chc_card.category }}"
  href="{{ chc_card.href }}"
  publish-date="{{ chc_card.publish_date }}"
  read-time="{{ chc_card.read_time }}"
  variant="{{ chc_card.variant }}"
>
  {% if chc_card.has_image %}
    <img
      slot="media"
      src="{{ chc_card.image_url }}"
      alt="{{ chc_card.image_alt }}"
      loading="lazy"
      decoding="async"
    />
  {% endif %}
...
This is the most common integration point. The article teaser maps Drupal's article content type to the `chc-content-card` component.
 ...
 *
 * Maps Drupal article fields to the chc-content-card Web Component.
 *
 * Required fields:
 *   - title (core)
 ...
 *
 * Uses chc-article-layout for page structure, with named slots
    ...
    {% if content.field_faq|render|trim is not empty %}
      <chc-accordion>
        {% for item in node.field_faq %}
          <chc-accordion-item heading="{{ item.field_question.value }}">
...
<chc-card-grid columns="{{ columns }}" gap="lg">
...
<chc-accordion>
  {% for item in paragraph.field_faq_items %}
    {% set faq = item.entity %}
    <chc-accordion-item heading="{{ faq.field_question.value }}">
...
<chc-tabs>
  {% for item in paragraph.field_tabs %}
    {% set tab = item.entity %}
    <chc-tab-item label="{{ tab.field_tab_label.value }}">
...
/**
 * @Block(
 *   id = "chc_featured_content",
 *   admin_label = @Translation("CHC Featured Content"),
 *   category = @Translation("CHC Components")
 * )
 */
...
{#
/**
 * Renders the article category field as a CHC badge.
 */
#}
{% for item in items %}
  <chc-badge variant="default" size="sm">
    {{ item.content }}
  </chc-badge>
{% endfor %}
```
...
{#
/**
 * Renders the article tags field as a row of CHC tags.
 */
#}
...
{#
/**
 * Renders testimonials as a grid of CHC cards.
 */
#}
{% if items|length > 0 %}
  <chc-card-grid columns="{{ items|length >= 3 ? '3' : items|length }}" gap="md">
    {% for item in items %}
      {% set testimonial = item.content['#paragraph'] %}
      <chc-content-card variant="default">
        <blockquote>
...
```php
<?php
// src/Plugin/Field/FieldFormatter/ChcContentCardFormatter.php
...
/**
 * Plugin implementation for rendering entity references as CHC content cards.
 *
 * @FieldFormatter(
 *   id = "chc_content_card",
 *   label = @Translation("CHC Content Card"),
 *   field_types = {"entity_reference"}
 * )
 */
class ChcContentCardFormatter extends FormatterBase {
...
Override the Views unformatted output to render results as a `chc-card-grid`:

```twig
{# views-view-unformatted--latest-articles.html.twig #}
{#
/**
 * Renders the "Latest Articles" view as a CHC card grid.
...
{#
/**
 * Renders a single view row as a chc-content-card.
 *
...
Override the Views pager to use the `chc-pagination` component:
...
```php
<?php
// src/Plugin/views/style/ChcCardGrid.php
...
/**
 * Views style plugin that renders results in a chc-card-grid.
 *
 * @ViewsStyle(
 *   id = "chc_card_grid",
 *   title = @Translation("CHC Card Grid"),
 *   help = @Translation("Renders view results as a CHC card grid Web Component."),
 *   theme = "views_view_chc_card_grid",
 *   display_types = {"normal"}
 * )
 */
class ChcCardGrid extends StylePluginBase {
...
```twig
{# views-view-chc-card-grid.html.twig #}
{{ attach_library('mytheme/hds-components') }}
...
/**
 * Builds a contact form using CHC Web Components.
 */
 ...
 *
 * Replace standard Drupal form elements with CHC Web Components
...
function mytheme_form_user_login_form_alter(array &$form, FormStateInterface $form_state): void {
  ...
  $form['actions']['submit']['#tag'] = 'chc-button';
...
The `chc-text-input`, `chc-textarea`, `chc-select`, `chc-checkbox`, and `chc-radio` components are **form-associated custom elements**. They use the `ElementInternals` API to participate natively in HTML forms.
...
<form method="post" action="/contact">
  ...
  <chc-text-input name="full_name" label="Full Name" required></chc-text-input>

  <chc-text-input name="email" label="Email" type="email" required></chc-text-input>

  <!-- On form submit, FormData contains: full_name=..., email=... -->
  <chc-button type="submit" variant="primary"> Submit </chc-button>
</form>
```
...
function mytheme_form_alter(array &$form, FormStateInterface $form_state): void {
  if ($form_state->hasAnyErrors()) {
    ...
    foreach ($errors as $field_name => $message) {
      ...
      if (isset($form[$field_name]) && $form[$field_name]['#tag'] ?? '' === 'chc-text-input') {
...
/**
 * @file
 * Drupal behaviors for CHC Web Component event handling.
 */
...
(function (Drupal, once) {
   ...
   */
  Drupal.behaviors.chcCardAnalytics = {
    attach(context) {
      const cards = once('chc-card-analytics', 'chc-content-card', context);
...
(function (Drupal) {
   ...
   * @type {Drupal~behavior}
   */
  Drupal.behaviors.chcAccordionTracking = {
    attach(context) {
      // Attach once to the document body, not individual components
      const containers = once('chc-accordion-delegate', 'body', context);

      containers.forEach((body) => {
        body.addEventListener('chc-accordion-toggle', (event) => {
          // event.composed is true, so it crosses shadow DOM boundaries
          const detail = event.detail;
          const accordion = event.target.closest('chc-accordion');
...
(function (Drupal, once) {
   ...
   * @type {Drupal~behavior}
   */
  Drupal.behaviors.chcPaginationAjax = {
    attach(context) {
      const pagers = once('chc-pagination-ajax', 'chc-pagination', context);

      pagers.forEach((pager) => {
        pager.addEventListener('chc-page-change', async (event) => {
...
Connect the `chc-search-bar` component to Drupal's Search API:
  ...
  Drupal.behaviors.chcSearch = {
    attach(context) {
      ...
      searchBars.forEach((searchBar) => {
        ...
        searchBar.addEventListener('chc-search-input', async (event) => {
          const { query } = event.detail;
...
(function (Drupal) {
   ...
   * @type {Drupal~behavior}
   */
  Drupal.behaviors.chcBigPipeHandler = {
    attach(context) {
...
(function (Drupal, once) {
   ...
   * @type {Drupal~behavior}
   */
  Drupal.behaviors.chcModals = {
    attach(context) {
      const triggers = once('chc-modal-trigger', '[data-chc-modal-target]', context);

      triggers.forEach((trigger) => {
        trigger.addEventListener('click', (event) => {
          event.preventDefault();
          const modalId = trigger.dataset.chcModalTarget;
          const modal = document.getElementById(modalId);

          if (modal && modal.tagName === 'CHC-MODAL') {
      ...
      const modals = once('chc-modal-events', 'chc-modal', context);
      modals.forEach((modal) => {
        modal.addEventListener('chc-modal-close', () => {
...
.hero-section chc-button::part(label) {
...
# content-card.component.yml
name: Content Card
description: 'Healthcare content card backed by the chc-content-card Web Component'
status: stable
props:
  type: object
  required:
...
(function (Drupal, once) {
   ...
   * @type {Drupal~behavior}
   */
  Drupal.behaviors.chcLazyLoad = {
    attach(context) {
      const lazyComponents = once('chc-lazy-load', '[data-chc-lazy]', context);
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              const componentModule = el.dataset.chcLazy;
...
<chc-media-gallery
  data-chc-lazy="/themes/custom/mytheme/node_modules/@org/wc-library/dist/components/media-gallery/index.js"
>
...
- [ ] Every `chc-text-input` has a visible, non-empty `label` attribute
...
**Symptom**: The page shows the raw HTML tags (e.g., `<chc-content-card>`) instead of rendered components.
...
const tags = ['chc-content-card', 'chc-button', 'chc-hero-banner'];
tags.forEach((tag) => {
...
```twig
{# WRONG: <chc-button disabled="false"> -- "false" is truthy in HTML #}
<chc-button {{ is_disabled ? 'disabled' : '' }}>

{# RIGHT: attribute present = true, absent = false #}
<chc-button {% if is_disabled %}disabled{% endif %}>
```
...
document.addEventListener('chc-card-click', (e) => console.log('card-click', e.detail));
document.addEventListener('chc-form-submit', (e) => console.log('form-submit', e.detail));

// Or listen for ALL events on a specific component
const card = document.querySelector('chc-content-card');
const events = ['chc-card-click'];
events.forEach((name) => {
...
inspectTokens(document.querySelector('chc-content-card')); // Check component tokens
```
...
2. **Use `:not(:defined)` CSS to hide unregistered elements**:
   ...
   ```css
   /* mytheme.css */
   chc-content-card:not(:defined),
   chc-hero-banner:not(:defined),
   chc-button:not(:defined) {
...
```twig
{# Before (v1) #}
<chc-content-card title="{{ node.label }}">

{# After (v2) -- attribute renamed #}
<chc-content-card heading="{{ node.label }}">
```
...
```javascript
// mytheme/js/chc-error-monitoring.js
...
- **Components used**: `chc-content-card`, `chc-card-grid`, `chc-hero-banner`, `chc-article-layout`, `chc-breadcrumb`, `chc-pagination`, `chc-button`, `chc-badge`, `chc-avatar`, `chc-accordion`, `chc-search-bar`
...
themes/custom/rhp_theme/
  ...
  js/
    chc-behaviors.js                       # Event handling behaviors
    chc-theme-toggle.js                    # Theme toggle behavior
  templates/
    html.html.twig                         # HTML wrapper with theme flash prevention
    page.html.twig                         # Page layout
    node--article--teaser.html.twig        # Article teaser -> chc-content-card
    node--article--full.html.twig          # Article full -> chc-article-layout
    field--node--field-category.html.twig  # Category field -> chc-badge
    field--node--field-tags.html.twig      # Tags field -> chc-tag
    views-view-unformatted--latest-articles.html.twig
    views-view-fields--latest-articles.html.twig
...
type: theme
description: 'Healthcare content hub theme using CHC Web Components'
core_version_requirement: ^10.3 || ^11
base theme: false
...
function rhp_theme_preprocess_node__article__full(array &$variables): void {
  ...
  if ($author) {
    ...
    if (!$author->get('field_avatar')->isEmpty()) {
      ...
      if ($avatar) {
        $variables['chc']['author']['avatar_url'] = \Drupal::service('file_url_generator')
...
<chc-content-card
  ...
  href="{{ chc.href }}"
  publish-date="{{ chc.publish_date }}"
  read-time="{{ chc.read_time }}"
  variant="{{ chc.variant }}"
>
  {% if content.field_media|render|trim is not empty %}
    <div slot="media">
...
{# Article layout #}
<chc-article-layout {% if chc.has_sidebar %}has-sidebar{% endif %}>
  {# Breadcrumb #}
  <nav slot="breadcrumb" aria-label="{{ 'Breadcrumb'|t }}">
    {{ drupal_block('system_breadcrumb_block') }}
  </nav>
  ...
  {# Author #}
  {% if chc.author is defined %}
    <div slot="author">
      <chc-media-object>
        {% if chc.author.avatar_url is defined %}
          <chc-avatar
            slot="media"
            src="{{ chc.author.avatar_url }}"
            alt="{{ chc.author.name }}"
            size="md"
          ></chc-avatar>
        {% endif %}
        <div>
          <strong>{{ chc.author.name }}</strong>
          <time datetime="{{ chc.date_iso }}">{{ chc.date }}</time>
          {% if chc.read_time %}
            <span>&middot; {{ chc.read_time }} {{ 'min read'|t }}</span>
          {% endif %}
        </div>
      </chc-media-object>
    </div>
...
/**
 * @file
 * Regional Health Partners -- CHC component behaviors.
 */
...
(function (Drupal, once) {
  ...
  Drupal.behaviors.rhpCardAnalytics = {
    attach(context) {
      const cards = once('rhp-card-analytics', 'chc-content-card', context);
  ...
  Drupal.behaviors.rhpSearch = {
    attach(context) {
      const searchBars = once('rhp-search', 'chc-search-bar', context);
  ...
  Drupal.behaviors.rhpAccordionTracking = {
    attach(context) {
      const accordions = once('rhp-accordion', 'chc-accordion', context);

      accordions.forEach((accordion) => {
        accordion.addEventListener('chc-accordion-toggle', (event) => {
          if (typeof gtag === 'function') {
...
- [ ] Custom elements are registered (verify with `customElements.get('chc-content-card')` in console)
...
This table maps every `chc-content-card` attribute to its Drupal source and TWIG expression. Equivalent tables for each component are available in the Storybook documentation.
...
| ---------------------- | ------------------ | ----------------------------- | ------------------------- |
| `chc-card-click`       | `chc-content-card` | `{ href, heading, keyboard }` | Analytics, navigation     |
| `chc-form-submit`      | `chc-form`         | `{ formData, valid }`         | Form processing           |
| `chc-input`            | `chc-text-input`   | `{ value, name }`             | Real-time validation      |
| `chc-change`           | `chc-text-input`   | `{ value, name }`             | Value change tracking     |
| `chc-search-submit`    | `chc-search-bar`   | `{ query }`                   | Search routing            |
| `chc-search-input`     | `chc-search-bar`   | `{ query }`                   | Autocomplete              |
| `chc-accordion-toggle` | `chc-accordion`    | `{ heading, open }`           | Analytics, state tracking |
| `chc-tab-change`       | `chc-tabs`         | `{ index, label }`            | Analytics, deep linking   |
| `chc-modal-open`       | `chc-modal`        | `{ id }`                      | Focus management          |
| `chc-modal-close`      | `chc-modal`        | `{ id }`                      | Focus restoration         |
| `chc-nav-toggle`       | `chc-nav-mobile`   | `{ open }`                    | Mobile nav state          |
| `chc-page-change`      | `chc-pagination`   | `{ page }`                    | AJAX paging               |
