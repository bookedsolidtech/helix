...
- **Workspace protocol**: Storybook references the library via `workspace:*`, always using the latest local build
- **Single CI pipeline**: One pipeline builds, tests, and validates everything

### Package Manager & Monorepo Tooling: npm + Turborepo

**Decision**: npm workspaces with Turborepo for task orchestration.

**Rationale**: npm workspaces with Turborepo's intelligent caching and task orchestration provide a robust monorepo foundation. npm is the default package manager shipped with Node.js, eliminating additional tooling setup. Turborepo adds smart task scheduling, remote caching, and dependency-aware build orchestration on top of npm workspaces. The `workspace:*` protocol expresses local package references cleanly.
...
Workspaces are defined in the root `package.json` via the `"workspaces"` field. Task orchestration is configured in `turbo.json`:
...
```json
{
  ...
  "scripts": {
    ...
    "test:ui": "turbo run test:ui --filter=@org/wc-library",
    "lint": "turbo run lint",
...
jobs:
  build-and-test:
    ...
    steps:
      ...
      - run: turbo run build:tokens
      - run: turbo run build
      - run: turbo run build:cem
      - run: turbo run test
      - run: turbo run storybook:build
  ...
  publish:
    ...
    steps:
      ...
      - run: npm ci
      - run: turbo run build:tokens && turbo run build && turbo run build:cem
