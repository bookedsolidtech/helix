...
wc-2026/
├── packages/
│   └── wc-library/          # @wc-2026/library — Lit 3.x components (CORE)
│       └── src/components/   # wc-button, wc-card, wc-text-input
├── apps/
...
- **Tag prefix**: `wc-` (e.g., `wc-button`, `wc-card`)
- **Event prefix**: `wc-` (e.g., `wc-click`, `wc-input`, `wc-change`)
- **CSS custom property prefix**: `--wc-` (e.g., `--wc-color-primary`)
...
- **File structure per component**:
  ...
  ├── wc-button.styles.ts   # Lit CSS tagged template
  ├── wc-button.stories.ts  # Storybook stories
  └── wc-button.test.ts     # Vitest browser tests
  ```
...
npm run type-check             # TypeScript strict (must pass before commit)
...
3. **Shadow DOM encapsulation** — Styles don't leak. Design tokens (`--wc-*`) are the only theming mechanism.
...
- Commit messages: concise, imperative ("Add wc-select component", not "Added")
...
| File | Purpose |
|------|---------|
| `packages/wc-library/src/components/` | Component source (delegate to `lit-specialist`) |
| `packages/wc-library/src/test-utils.ts` | Shared test helpers (fixture, shadowQuery, oneEvent, cleanup) |
| `packages/wc-library/custom-elements.json` | CEM output (generated, don't edit) |
