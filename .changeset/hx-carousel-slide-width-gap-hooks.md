...
Measured navigation is derived from the rendered geometry (each slide's real leading-edge offset and the track's actual scroll extent), so slides of differing sizes are each reached and land flush, the last slide is flush at the trailing edge, and over/under-scroll never occurs. For uniform slides this reduces exactly to the previous per-page math.
