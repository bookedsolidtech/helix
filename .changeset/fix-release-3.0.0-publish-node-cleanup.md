...
- Drop Node 20 from `.nvmrc` (→22), root `package.json` engines, `packages/helixui-mcp/package.json` engines, and all GitHub Actions workflows (`ci.yml`, `publish.yml`, `release.yml`, `canary.yml`, `audit-batch-ci.yml`, `cross-browser.yml`). Node 20 reached EOL — supported runtimes are now Node 22 and Node 24.
