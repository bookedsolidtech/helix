...
<EyebrowHeading
  eyebrow="HELiX Storybook · @helixui/library@3.11.2 · @helixui/icons@1.1.0"
...
<section>
  ...
  <p className="hx-docs-card-body" style={{ marginTop: 16, maxWidth: '70ch' }}>
    Measured by the formal AAA harness (<code>pnpm aaa:audit</code>) against each P0
    component&apos;s audit story — many components use a curated <code>AaaAudit</code> story rather
    than the bare <code>Default</code> so the harness measures the canonical AAA-shaped variant.
    Evidence is recorded per (component × criterion) under <code>.reports/formal-aaa-audit/</code>{' '}
    (gitignored, reproducible via the harness) and the committed verdicts ship in{' '}
    <code>packages/hx-library/aaa-verdicts.json</code>. This cert is self-certified by the project
  ...
  <div className="hx-docs-pillars">
    ...
    <article className="hx-docs-pillar">
      ...
      <p>
        ...
        <a href="https://helix.bookedsolid.tech/drupal/installation/getting-started/">
          Drupal integration
        </a>
        .
      </p>
  ...
  <div className="hx-docs-pillars">
    ...
    <article className="hx-docs-pillar">
      ...
      <p>
        ...
        <code>@helixui/icons@1.1.0</code> registry), <strong>Layout</strong>.
      </p>
    </article>
    <article className="hx-docs-pillar">
      ...
      <p>
        ...
        parts, CSS custom properties, events. Inspect rendered focus / hover / active states via the
        Storybook pseudo-state toolbar on each story.
      </p>
    </article>
  </div>
</section>
...
<section>
  ...
  <CodeBlock
    ...
    code={`# install — @helixui/library auto-resolves @helixui/tokens + lit
# (dependencies) and requires @helixui/icons + @floating-ui/dom as
# peer dependencies, so install those alongside the library:
npm i @helixui/library @helixui/icons @floating-ui/dom
