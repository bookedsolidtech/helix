class WcPinGate extends HTMLElement {
  ...
  constructor() {
    ...
    const errorEl = this.querySelector('#pin-error');
    const canvas = this.querySelector('.pin-particles');

    if (!errorEl || !canvas) {
      throw new Error('WcPinGate: Required elements not found');
    }
  ...
  bindEvents(): void {
    ...
    document.addEventListener(
      ...
      true,
    ); // CAPTURE phase -- fires before Starlight search etc.
  }
  ...
  initParticles(): void {
    ...
    for (let i = 0; i < count; i++) {
      const colorIndex = Math.floor(Math.random() * idx);
      this.particles.push({
        x: Math.random() * w,
        y: Math.random() * h,
