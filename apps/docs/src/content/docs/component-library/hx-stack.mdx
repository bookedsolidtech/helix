...
| `inline`    | `inline`    | `boolean`                                                           | `false`      | When true, renders as `display: inline-flex` rather than `flex`. Reflects to attribute.      |
...
| -------------- | --------- | ----------------------------------------- |
| `--hx-space-1` | `0.25rem` | Gap token used when `gap="xs"`.           |
| `--hx-space-2` | `0.5rem`  | Gap token used when `gap="sm"`.           |
| `--hx-space-4` | `1rem`    | Gap token used when `gap="md"` (default). |
| `--hx-space-6` | `1.5rem`  | Gap token used when `gap="lg"`.           |
| `--hx-space-8` | `2rem`    | Gap token used when `gap="xl"`.           |

Override the global token scale for your theme:

```css
:root {
  --hx-space-4: 1.25rem; /* tighten or loosen default gap globally */
}
```
