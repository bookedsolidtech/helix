...
Groups with one or more disabled buttons maintain visual cohesion.

<ComponentDemo title="Disabled Children">
  <div style="display: flex; flex-direction: column; gap: 1rem;">
    <hx-button-group accessible-label="Actions with disabled">
      <hx-button variant="secondary">Edit</hx-button>
    ...
    <hx-button-group accessible-label="All disabled">
      <hx-button variant="secondary" disabled>
        Edit
      </hx-button>
      <hx-button variant="secondary" disabled>
...
<ComponentDemo title="Patient Record Actions">
  <div style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: center;">
    <hx-button-group accessible-label="Chart view controls">
    ...
    <hx-button-group accessible-label="Record actions">
...
| `size`        | `hx-size`     | `'sm' \| 'md' \| 'lg'`       | `'md'`         | Sets `--hx-button-group-size` CSS custom property on the group host. Child `hx-button` elements should also set `hx-size` until automatic cascade is implemented. |
...
| Reduced motion | Child `hx-button` components handle their own `prefers-reduced-motion` behavior internally (e.g., suppressing spinner animations).                                                                |
...
<hx-button-group
  ...
  {% if label %}accessible-label="{{ label }}"{% endif %}
>
...
<html lang="en">
  ...
  >
    ...
    <hx-button-group accessible-label="Form actions">
    ...
    <!-- Vertical group -->
    <hx-button-group orientation="vertical" accessible-label="Navigation options">
    ...
    <!-- Small size group -->
    <hx-button-group hx-size="sm" accessible-label="View controls">
