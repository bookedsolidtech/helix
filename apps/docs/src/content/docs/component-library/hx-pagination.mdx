...
| `labelRowsPerPage`  | `label-rows-per-page` | `string`                                     | `'Rows per page:'` | Label text for the rows-per-page selector. Override for i18n.                                                                  |
| `firstPageLabel`    | `first-page-label`    | `string`                                     | `'First page'`     | Accessible label for the First page (`«`) navigation button. Override for i18n.                                                |
| `previousPageLabel` | `previous-page-label` | `string`                                     | `'Previous page'`  | Accessible label for the Previous page (`‹`) navigation button. Override for i18n.                                             |
| `nextPageLabel`     | `next-page-label`     | `string`                                     | `'Next page'`      | Accessible label for the Next page (`›`) navigation button. Override for i18n.                                                 |
| `lastPageLabel`     | `last-page-label`     | `string`                                     | `'Last page'`      | Accessible label for the Last page (`»`) navigation button. Override for i18n.                                                 |
| `labelPageMessage`  | —                     | `(current: number, total: number) => string` | `'Page N of N'`    | Function returning the `aria-live` announcement after each navigation. Override for i18n. Not configurable via HTML attribute. |
| `labelPageButton`   | —                     | `(page: number) => string`                   | `'Page N'`         | Function returning the `aria-label` for each page button. Override for i18n. Not configurable via HTML attribute.              |
