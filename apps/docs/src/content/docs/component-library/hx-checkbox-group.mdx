...
Group-level help text via the `help-text` slot.
...
<ComponentDemo title="With Help Text">
  <hx-checkbox-group label="Appointment reminders" name="reminders">
    ...
    <span slot="help-text">You will receive reminders for each selected interval.</span>
  </hx-checkbox-group>
</ComponentDemo>
...
| `help-text` | Group-level help text rendered below the checkbox list.                                     |
...
| Required state | A visual `*` marker is rendered in the legend with `aria-hidden="true"`. The `required` attribute reflects on the host element. `aria-required` is not placed on the `<fieldset>` because axe-core flags it as `aria-allowed-attr` violation (the `group` role does not permit `aria-required` in the current ARIA spec). The form constraint API (`ElementInternals.setValidity`) signals invalidity programmatically. |
  ...
  <body style="font-family: sans-serif; padding: 2rem; max-width: 480px;">
    <form id="preferences-form">
      <hx-checkbox-group id="channels" label="Notification channels" name="channels" required>
        ...
        <hx-checkbox value="sms" label="SMS"></hx-checkbox>
        <hx-checkbox value="push" label="Push notification"></hx-checkbox>
        <span slot="help-text">Select all channels you want to receive alerts on.</span>
      </hx-checkbox-group>
