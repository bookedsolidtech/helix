...
<ComponentDemo title="Vertical Steps">
  <hx-steps orientation="vertical" accessible-label="Patient intake progress">
    <hx-step
      label="Demographics"
      status="complete"
...
<ComponentDemo title="Size Variants">
  <div style="display: flex; flex-direction: column; gap: 2rem;">
    <div>
      ...
      <hx-steps size="sm" accessible-label="Small steps">
    ...
    <div>
      ...
      <hx-steps size="md" accessible-label="Medium steps">
        <hx-step label="Step 1" status="complete"></hx-step>
        <hx-step label="Step 2" status="active"></hx-step>
        <hx-step label="Step 3" status="pending"></hx-step>
      </hx-steps>
    ...
    <div>
      ...
      <hx-steps size="lg" accessible-label="Large steps">
        <hx-step label="Step 1" status="complete"></hx-step>
        <hx-step label="Step 2" status="active"></hx-step>
        <hx-step label="Step 3" status="pending"></hx-step>
      </hx-steps>
...
<ComponentDemo title="All Status Variants">
  <hx-steps accessible-label="All step statuses">
...
<ComponentDemo title="Disabled Steps">
  <hx-steps accessible-label="Authorization workflow">
...
<ComponentDemo title="Prior Authorization Flow">
  <hx-steps orientation="vertical" accessible-label="Prior authorization submission">
    <hx-step
...
<!-- Vertical orientation -->
<hx-steps orientation="vertical" accessible-label="Patient intake">
...
<hx-steps size="lg" accessible-label="Authorization steps">
...
| `size`            | `hx-size`          | `'sm' \| 'md' \| 'lg'`       | `'md'`         | Size variant. Propagated automatically to all child `<hx-step>` elements. Reflects to attribute. Use the `hx-size` attribute in HTML; the legacy `size` attribute is accepted for backward compatibility but is deprecated. |
| `accessibleLabel` | `accessible-label` | `string \| null`             | `null`         | Accessible label for the steps list. Always provide a descriptive value that names the workflow (e.g., `"Checkout progress"`). Forwarded to the inner `role="list"` element.                                                |
...
```html
<!-- Custom icon slot -->
<hx-steps accessible-label="Upload progress">
...
| `accessible-label` | **Always provide `accessible-label` on `<hx-steps>`** describing the workflow (e.g., `"Checkout progress"`, `"Prior authorization steps"`). Without it, the list is announced without context. |
...
Always provide a descriptive `accessible-label` on `<hx-steps>` that names the overall process:

```html
<!-- Preferred: process-specific label -->
<hx-steps accessible-label="Prior authorization submission">...</hx-steps>
<hx-steps accessible-label="Patient intake workflow">...</hx-steps>
<hx-steps accessible-label="Checkout progress">...</hx-steps>

<!-- Acceptable: generic label -->
<hx-steps accessible-label="Progress steps">...</hx-steps>
...
<hx-steps
  ...
  accessible-label="{{ accessible_label|default('Progress steps') }}"
>
  {% for step in steps %}
    <hx-step
      label="{{ step.label }}"
...
<html lang="en">
  ...
  >
    ...
    <section>
      ...
      <hx-steps id="checkout-steps" accessible-label="Checkout progress">
    ...
    <section>
      ...
      <hx-steps orientation="vertical" accessible-label="Patient intake workflow">
        <hx-step
          label="Demographics"
          status="complete"
          description="Personal information collected"
    ...
    <section>
      ...
      <hx-steps id="nav-steps" accessible-label="Form sections">
