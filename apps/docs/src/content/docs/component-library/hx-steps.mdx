...
| `ariaLabel`   | `aria-label`  | `string \| null`             | `null`         | Accessible label for the steps list. Always provide a descriptive value that names the workflow (e.g., `"Checkout progress"`). Forwarded to the inner `role="list"` element.                                                |
