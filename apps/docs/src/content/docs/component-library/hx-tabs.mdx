...
The component fully implements the [WAI-ARIA Tabs Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/) with roving `tabindex`, `aria-selected`, `aria-controls`, and both automatic and manual keyboard activation modes.
...
| `aria-label`          | Set on each `<hx-tab-panel>` using the label text of its corresponding `<hx-tab>`. Used instead of `aria-labelledby` because the tab and panel live in different shadow DOM trees, making cross-boundary `id` references unreliable. |
