...
Audit any forked nav template (and derived `hx-top-nav` / `hx-side-nav` usages) for the same mistake.
...
The library strips a `value` attribute off `hx-phi-field` in `connectedCallback` to prevent PHI retention in `outerHTML` / DevTools after hydration (see §6). The starter's `hx-phi-field.twig` does **not** render `value` as an HTML attribute — PHI is passed to the component via the `data` JS property, set client-side through a Drupal behavior.

If a forked template emits PHI into the `value` or `data` attribute from the server, the runtime rescue can remove it from the live DOM but **cannot** remove it from the SSR HTML, the HTTP response body, `View Source`, browser caches, or any access logs that recorded the response. The supported pattern for Drupal:
...
PHI only ever crosses the wire inside `drupalSettings` (a JS-readable payload), not as an HTML attribute. Combine with your site's existing role-based access checks and audit logging on the `hx-phi-access` event.
