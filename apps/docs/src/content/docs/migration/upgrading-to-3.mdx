...
## 4. `hx-date-picker` / `hx-time-picker` popup is still a focus-trapping dialog

The calendar / clock popup continues to render a native `<dialog>` element following the W3C APG
...
// After — class types use Helix* naming; CustomEvent detail types
// and a small set of legacy re-exports retain Hx* naming.
import type { HelixButton, HelixCard, HxSwitch } from '@helixui/library';

// Event-detail types use Hx* naming:
import type { HxButtonClickDetail, HxSwitchChangeDetail } from '@helixui/library';
```
...
`hx-popover`, `hx-dropdown`, `hx-overflow-menu`, `hx-popup`, and `hx-tooltip` load
