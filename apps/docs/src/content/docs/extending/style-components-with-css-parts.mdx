...
For HELiX's supported browser baseline — see [`BROWSER_COMPATIBILITY.md`](https://github.com/bookedsolidtech/helix/blob/main/packages/hx-library/BROWSER_COMPATIBILITY.md) for the authoritative matrix; current targets are Chromium/Edge/Firefox 120+ and Safari/Safari iOS 17+ — `::part()` can be used without any polyfill or fallback.
