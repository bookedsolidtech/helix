...
**Conventions used in every HELiX `play()` block:**
...
- `canvasElement.querySelector('hx-*')` for the host. `host.shadowRoot!.querySelector(...)`
  for the focusable native control. This is the documented test-utility
  pattern in `packages/hx-library/src/test-utils.ts`.
- Listener cleanup at the end of every block (`removeEventListener`) so
  re-runs do not double-count.
- Assertions on `bubbles` + `composed` for every `hx-*` event — the
  shadow-boundary composition contract is load-bearing.
...
export const SubmitEmpty: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    let submitted = false;
    canvasElement.querySelector('#intake')!.addEventListener('hx-submit', () => {
      submitted = true;
    });
    ...
    // Empty required fields must block submission.
...
- `patient-intake.stories.ts` — `hx-form` validation + happy-path submission
