...
**Result**: The `hx-tokens` package generates `--hx-*` CSS custom properties via a TypeScript generator script. All 81 components consume tokens through the three-tier fallback pattern: `var(--hx-component-token, var(--hx-semantic-token))`.
