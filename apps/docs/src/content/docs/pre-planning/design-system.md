...
9. [Accessibility-First Architecture](#39-accessibility-first-architecture)
...
This section defines the design system architecture for an enterprise-grade Web Component library targeting content organizations. The architecture is built on three foundational pillars:
...
**Why this matters for enterprise**: Organizations face strict accessibility requirements (WCAG 2.1/2.2 AAA for critical content), diverse user populations (users with low vision, motor impairments, cognitive differences), and enterprise brand management needs. A well-architected token system makes it possible to deliver accessible, branded, consistent UI across an entire content platform without per-component overrides.
...
- **OKLCH color space**: We use OKLCH (Oklab Lightness, Chroma, Hue) as the canonical color format. OKLCH is perceptually uniform -- a lightness value of 0.50 looks equally bright regardless of hue. This is critical for accessibility because it makes contrast ratio calculations predictable and palette generation mathematically consistent. The DTCG 2025.10 spec supports OKLCH natively, and Terrazzo/Style Dictionary handle automatic downconversion to hex/rgb for older browsers.
...
- **Major Third type scale (1.250)**: A conservative ratio that produces readable, well-differentiated sizes without extreme jumps -- appropriate for content platforms where readability is paramount.
...
- **Fallback-first**: Component tokens alias semantic tokens by default. An organization can override `--button-primary-bg` without overriding `--color-interactive-primary`, and vice versa.
...
7. **Prefix CSS custom properties with a namespace**: `--hds-color-surface-primary` (where `hds` = HELIX Design System, configurable per client)
...
```css
/* Default namespace: hds (HELIX Design System) */
:root {
...
export class HdsButton extends LitElement {
  static styles = css`
    ...
    button {
      ...
      /* Minimum touch target for WCAG accessibility */
      min-height: 44px;
      min-width: 44px;
    }
...
We use a **Major Third** scale (ratio: 1.250) anchored at 1rem (16px). This ratio provides clear visual hierarchy without extreme size jumps -- appropriate for content platforms where readability is the priority over dramatic display typography.
...
2. **`font-display: swap`**: Text renders immediately in a fallback system font, then swaps when the custom font loads. This is the right choice for content platforms where readability is more important than preventing a brief style shift.

3. **Unicode subsetting**: Only load the character ranges needed for the content language. Latin base covers 99%+ of English content.
...
Content platforms demand generous line height for readability:
...
**Container query browser support** (early 2026): 93%+ global support. For legacy Safari gaps, a polyfill exists but is unlikely to be needed for an enterprise deployment in 2026.
...
## 3.9 Accessibility-First Architecture

### WCAG Compliance Targets

| Category                    | Target Level                         | Rationale                                     |
| --------------------------- | ------------------------------------ | --------------------------------------------- |
| Color contrast (text)       | **AAA (7:1)**                        | Users include elderly and low-vision visitors |
...
Focus styles are critical for enterprise applications where keyboard navigation may be the only option:
...
const meta: Meta = {
  ...
  parameters: {
    a11y: {
      config: {
        rules: [
          // Enforce AAA contrast for enterprise accessibility
...
/* mytheme.css */
:root {
  /* Override primary brand color for this organization */
