...
**Slot content is light DOM.** Content passed into slots lives in the document's DOM, not inside the component's shadow root. This means it inherits your application styles but not the component's internal styles. Use the [adopted-stylesheets pattern](/extending/adopted-stylesheets/) when slotted content needs base typography, link styles, or CMS-authored formatting.
