...
`hx-theme` renders as a `display: contents` wrapper that adds the named attribute and hosts the scoped token overrides. See the [hx-theme component docs](https://storybook.helix.bookedsolid.tech/?path=/docs/components-theme--docs) for available built-in theme names and how to register custom themes.
