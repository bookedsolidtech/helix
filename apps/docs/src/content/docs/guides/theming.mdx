...
`hx-theme` renders as a `display: contents` wrapper that adds the named attribute and hosts the scoped token overrides. See the [hx-theme Storybook docs](https://storybook.helix.bookedsolid.tech/?path=/docs/components-hx-theme--docs) for available built-in theme names and how to register custom themes.
