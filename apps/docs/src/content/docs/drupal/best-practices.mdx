...
<Aside type="tip" title="Best Practice">
  Use CDN loading for production to benefit from browser caching and HTTP/2
  multiplexing. Pin to a specific version (e.g., `@1.0.0`) to prevent breaking
...
<Aside type="note" title="Reference">
  See **ADR-DRUPAL-001** in the HELiX documentation for detailed guidance on
  choosing between property-driven and slot-driven patterns.
</Aside>
...
(function (Drupal, once) {
  ...
  function updateSpecialtyOptions(departmentId) {
    const specialtySelect = document.querySelector(
      'hx-select[name="specialty"]',
    );
...
<Aside type="note" title="Reference">
  See **ADR-DRUPAL-002** for event handling best practices, including custom
  event naming conventions and cleanup patterns.
</Aside>
...
<Aside type="note" title="Reference">
  See **ADR-DRUPAL-003** for comprehensive performance optimization strategies,
  including HTTP/2 server push, asset aggregation, and lazy loading patterns.
</Aside>
...
<Aside type="note" title="Reference">
  See **ADR-DRUPAL-004** for accessibility best practices, including ARIA
  patterns, focus management, and screen reader compatibility in Drupal
  workflows.
</Aside>
...
<Aside type="tip" title="Next Steps">
  Review individual component documentation (hx-card, hx-select, hx-form) for
  specific Drupal integration examples. See the visual ADRs in the HELiX
