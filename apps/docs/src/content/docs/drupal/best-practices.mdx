...
<Aside type="tip" title="Best Practice">
  ...
  specific version (e.g., `@3.11.2`) to prevent breaking changes from auto-updating.
</Aside>
...
**Library definition with SRI** — the `integrity` value below is a placeholder. SRI digests are
specific to one exact file, so paste the digest you generated above and regenerate it every time you
change the pinned version, or the browser will refuse to execute the script:
  ...
  js:
    https://cdn.jsdelivr.net/npm/@helixui/library@3.11.2/dist/index.js:
      ...
      attributes:
        ...
        # sha384 digest from the command above — regenerate it whenever you change the pinned version
        integrity: sha384-REPLACE_WITH_DIGEST_FOR_THE_VERSION_YOU_PIN
```
