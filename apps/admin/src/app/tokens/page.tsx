...
export default function TokensDashboard() {
  ...
  const referencingTokens = allTokens.filter((t) =>
    t.value.includes("var(")
  ).length;
  const coveragePercent =
    allTokens.length > 0
      ? Math.round((referencingTokens / allTokens.length) * 100)
      : 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={<Hash className="w-3.5 h-3.5" />} label="Total Tokens" value={allTokens.length} />
        <StatCard icon={<Layers className="w-3.5 h-3.5" />} label="Categories" value={Object.keys(stats.byCategory).length} />
        <StatCard icon={<Link2 className="w-3.5 h-3.5" />} label="Cross-References" value={referencingTokens} />
        <Card>
          ...
          <CardContent>
            <Badge variant="secondary" className="text-xs">JSON (DTCG)</Badge>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">
            Token Cross-Reference Coverage
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-muted-foreground">
                  {referencingTokens} of {allTokens.length} tokens reference
                  other tokens via <code className="font-mono">var()</code>
                </span>
