...
import { FileCode2, Cpu, FileJson2, BookOpen, Globe } from 'lucide-react';
    ...
    <div className="flex items-center flex-1 min-w-0">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-0.5">
          {node.count !== undefined && (
            <span className="text-xs text-muted-foreground tabular-nums">{node.count} items</span>
          )}
          {node.detail && (
            <span className="text-[11px] text-muted-foreground/70 text-center">{node.detail}</span>
          )}
        </div>
      </div>
      ...
      {!isLast && (
        <div className="w-8 shrink-0 flex items-center justify-center relative">
          <div className={cn('h-px w-full bg-gradient-to-r to-transparent', s.line)} />
          <div className={cn('absolute right-0 w-1.5 h-1.5 rounded-full', s.dot, 'opacity-60')} />
        </div>
      )}
    </div>
  );
...
export function PipelineFlow({ nodes }: PipelineFlowProps) {
  return (
    <div className="flex items-stretch gap-0 overflow-x-auto py-4 px-1 -my-4 -mx-1">
      {nodes.map((node, i) => (
        <PipelineNodeCard key={node.label} node={node} index={i} isLast={i === nodes.length - 1} />
      ))}
    </div>
  );
}
  ...
  return (
    <div
      className={cn(
        ...
        healthy ? 'bg-emerald-400 ring-emerald-400/20' : 'bg-red-400 ring-red-400/20',
      )}
    />
  );
}
