...
You are conducting a ruthless, evidence-based technical audit of **wc-2026** at `/workspace/.`. This is a Next.js 15.3.3 project using TypeScript (strict mode). Quality, security, and reliability are non-negotiable.
...
ONLY review `/workspace/.`. Do not review files outside this scope unless they are direct imports from shared packages referenced by the target.
...
Write to: `/workspace/.workstream-results/wc-2026-cross-cutting-issues.jsonl`
...
Write to: `/workspace/.workstream-results/wc-2026-cross-cutting-report.md` using this exact format:
...
## ADDITIONAL: Monorepo Cross-Cutting Concerns

In addition to the standard review, check:\n- Dependency version consistency across packages\n- Shared configuration consistency (tsconfig, eslint, prettier)\n- Build pipeline correctness (proper task ordering, caching)\n- Package boundary violations (imports bypassing public API)\n- CI/CD pipeline completeness (all packages tested and built)\n- Changelog and versioning strategy\n
```
